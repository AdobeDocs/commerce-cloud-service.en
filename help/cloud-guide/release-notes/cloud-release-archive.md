...
-  ![new icon](../../assets/new.svg) **New environment variables**—
   ...
   -  **[X_FRAME_CONFIGURATION global variable](../environment/variables-global.md#x_frame_configuration)**—Use this variable to change the `X-Frame-Options` header configuration for rendering an Adobe Commerce page in a `<frame>`, `<iframe>`, or `<object>`.<!-- MAGECLOUD-3048 -->
...
-  ![fix icon](../../assets/fix.svg) Fixed a deployment error that occurs when the `.magento.env.yaml` contains `{{ base_url }}` and `{{ unsecure_base_url }}` placeholders for web configurations instead of the default URL configuration for an Adobe Commerce on cloud infrastructure project./<!--MAGECLOUD-2607-->
