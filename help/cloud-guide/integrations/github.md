...
| ----------------------- | --------- | --------------------------------- |
| `--base-url`            | Yes       | The base URL of the server installation, which may be `https://github.com/` or a custom. Omit this option if your repository is hosted with public Github or if your repository is not hosted on private servers. Omit this option if your repository url is similar to `https://github.com/{account}/{repository-name}`. This can cause error like `Unable to connect to GitHub: repository not found`.  |
