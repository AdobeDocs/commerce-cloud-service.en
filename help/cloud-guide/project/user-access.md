...
1. After adding users, redeploy all environments to apply the changes. **_Adding a user does not trigger a deployment automatically._** Redeployment is an important step to ensure that the user can access an environment using SSH or perform administrator tasks.
